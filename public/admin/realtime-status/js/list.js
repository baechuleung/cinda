...
import { collection, getDocs, doc, getDoc, deleteDoc, updateDoc, query, orderBy } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
...
function displayStores(stores) {
    ...
    tbody.innerHTML = stores.map(store => `
        <tr>
            <td>
                ${store.storeName}
                ${store.storeCode ? `<br><small style="color: #666;">${store.storeCode}</small>` : ''}
            </td>
...
window.deleteStore = async function(storeId) {
    ...
    try {
        ...
        if (storeCode) {
            ...
            if (masterDoc.exists()) {
                const codes = masterDoc.data().codes || {};
                delete codes[storeCode];
                
                await updateDoc(doc(db, 'store_codes', 'master'), { codes: codes });
            }
        }
...
function filterStores() {
    ...
    const filteredStores = allStores.filter(store => {
        const matchSearch = store.storeName.toLowerCase().includes(searchTerm) ||
                          ...
                          (store.storeCode && store.storeCode.toLowerCase().includes(searchTerm));
